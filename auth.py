...
INPUTS
    - permission: string permission (i.e 'post: item')
...
- raises an AuthError if the requested permission string is not in the payload
  permission array & returns true otherwise.
'''
...
'''
implements a verification decode jwt token method which INPUTS:
...
Common certificate error described here:
...
'''
Implements @requires_auth(permission) decorator method which inputs:
    - permission: string permission (example: 'post:item').
- Uses the get_token_auth_header method to get the token.
- Uses the verify_decode_jwt method to decode the jwt.
- Uses the check_permissions method to validate claims and check the requested
  permission.
- Then returns the decorator which passes the decoded payload to the decorated
  method.
'''
